...
const SessionSidebar: React.FC<SessionSidebarProps> = ({
    ...
    // Optimized Resize logic using requestAnimationFrame
        ...
        const handleMouseMove = (e: MouseEvent) => {
            ...
            if (animationFrameId.current === null) {
                animationFrameId.current = requestAnimationFrame(() => {
                    ...
                    const newWidth = Math.max(200, Math.min(600, clientXRef.current)); // Min 200, Max 600
                    setWidth(newWidth);
                });
            }
        };
    ...
    useEffect(() => {
        ...
        const openDrawer = async () => {
            (drawer as any).showPopover();

            if (!CSS.supports('scroll-initial-target', 'nearest')) {
                // Jump-scroll to the closed stop so the scroll below animates the drawer in from off-screen.
                scroller.scrollTo({ left: scroller.offsetWidth, behavior: 'instant' as any });
        ...
        // Scroll fallback for background opacity if timeline is not supported
        const handleScroll = () => {
        ...
        return () => {
            ...
            observer.disconnect();
            // Cleanup inert on mobile drawer close/unmount
            const mainContent = document.getElementById('main-content');
            if (mainContent) {
                mainContent.removeAttribute('inert');
            }
    ...
    const getSyncIcon = (session: SessionWithStatus) => {
        ...
        if (session.syncStatus === 'offline') {
            return <FaCloudUploadAlt className="text-warning ms-1" title="En attente de synchronisation" size={12} role="img" />;
        }
        if (session.ownerId && user && session.ownerId !== user.uid) {
             return <FaUserFriends className="text-info ms-1" title="Partagé avec vous" size={12} role="img" />;
        }
        if (session.sharedWith && session.sharedWith.length > 0) {
             return <FaUserFriends className="text-primary ms-1" title="Partagé par vous" size={12} role="img" />;
        }

        return <FaCloud className="text-success ms-1" title="Synchronisé" size={12} role="img" />;
    };

    // Responsive Mobile Drawer View (< 768px)
    if (isMobile) {
        return (
            <div className="Drawer" id="drawer" {...{ popover: "manual" }} ref={drawerRef}>
                <div className="Drawer-scroller" ref={scrollerRef}>
                    <div className="Drawer-sheet" tabIndex={-1} ref={sheetRef}>
                        ...
                        <div className="sidebar-content">
                            ...
                            <div className="mt-4 px-3">
                                 ...
                                 <div className="sidebar-item" onClick={() => {
                                     onOpenTemplateEditor();
                                     scrollerRef.current?.scrollTo({ left: scrollerRef.current.offsetWidth, behavior: 'auto' });
                                 }} role="button" aria-label="Modèles d'emails">
                                    <span className="d-flex align-items-center gap-2">
                                         <FaEnvelope size={14} aria-hidden="true" /> Modèles d'emails
                                    </span>
                                 </div>
                            </div>
                        </div>
                        ...
                        <div className="sidebar-footer">
                            {!isOnline && (
                                <Alert variant="warning" className="m-2 p-2 small">
                                    <FaWifi className="me-2" aria-hidden="true" /> Mode hors ligne. Vos modifications sont sauvegardées localement.
                                </Alert>
                            )}
                            {!user && isOnline && (
                                ...
                                {user ? (
                                    ...
                                    <div className="d-flex flex-column gap-2">
                                        <Button variant="outline-primary" size="sm" className="w-100 d-flex align-items-center justify-content-center gap-2" onClick={() => login()}>
                                            <FaGoogle aria-hidden="true" /> Se connecter
                                        </Button>
                ...
                <Modal show={showShareModal} onHide={() => setShowShareModal(false)} centered>
                    ...
                    <Modal.Body>
                        ...
                        <Form.Group controlId="shareEmailMobile">
                         ...
                         {!isOnline && (
                             <OverlayTrigger placement="right" overlay={<Tooltip>Mode hors ligne</Tooltip>}>
                                 <div className="text-warning">
                                     <FaWifi size={16} aria-hidden="true" />
                                 </div>
                             </OverlayTrigger>
                         )}
                    </div>
    ...
    return (
        <div className="sidebar-container" style={{ width }}>
            ...
            <Modal show={showShareModal} onHide={() => setShowShareModal(false)} centered>
                ...
                <Modal.Body>
                    <p>Entrez l'adresse email de l'utilisateur avec qui vous souhaitez partager <strong>{sessionToShare?.jobTitle}</strong>.</p>
                    <Form.Group controlId="shareEmailDesktop">
                        <Form.Label>Email</Form.Label>
                        <Form.Control
                            type="email"
                            placeholder="exemple@email.com"
